...
load(sgbow_designs);
responses = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/response2_100.mat');
load(responses);
...
load(sgbow_lo);
response_lo = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/lo_response2_100.mat');
load(response_lo);

N = size(response2_100, 1); 
ncomp = 25;
correct = struct();
correct.('context') = 0;
correct.('order') = 0;
...
for i = 1:N
    ...
    y_matrix = squeeze(zscore(response2_100(i,:, :)));
    ...
    [XL_contextsgbow,YL_contextsgbow,XS_contextsgbow,YS_contextsgbow,BETA_contextsgbow,PCTVAR_contextsgbow] = plsregress(x_contextsgbow,y_matrix,ncomp);
    [XL_contextordersgbow,YL_contextordersgbow,XS_contextordersgbow,YS_contextordersgbow,BETA_contextordersgbow,PCTVAR_contextordersgbow] = plsregress(x_contextordersgbow,y_matrix,ncomp);
    %pctvar = pctvar + PCTVAR;
    
    %get the values of the semantic features that will be used to predict activation for word1 and word2

    context_pred1 = [1 squeeze(zscore(lo_context_svd(i,1,:)))']*BETA_context;
    context_pred2 = [1 squeeze(zscore(lo_context_svd(i,2,:)))']*BETA_context;
    order_pred1 = [1 squeeze(zscore(lo_order_svd(i,1,:)))']*BETA_order;
    order_pred2 = [1 squeeze(zscore(lo_order_svd(i,2,:)))']*BETA_order;
    contextorder_pred1 = [1 squeeze(zscore(lo_contextorder_svd(i,1,:)))']*BETA_contextorder;
    contextorder_pred2 = [1 squeeze(zscore(lo_contextorder_svd(i,2,:)))']*BETA_contextorder;
    sg_pred1 = [1 squeeze(zscore(lo_sg_svd(i,1,:)))']*BETA_sg;
    sg_pred2 = [1 squeeze(zscore(lo_sg_svd(i,2,:)))']*BETA_sg;
    bow_pred1 = [1 squeeze(zscore(lo_bow_svd(i,1,:)))']*BETA_bow;
    bow_pred2 = [1 squeeze(zscore(lo_bow_svd(i,2,:)))']*BETA_bow;
    sgbow_pred1 = [1 squeeze(zscore(lo_sgbow_svd(i,1,:)))']*BETA_sgbow;
    sgbow_pred2 = [1 squeeze(zscore(lo_sgbow_svd(i,2,:)))']*BETA_sgbow;
    contextsgbow_pred1 = [1 squeeze(zscore(lo_context_svd(i,1,:)))' squeeze(zscore(lo_sg_svd(i,1,:)))' squeeze(zscore(lo_bow_svd(i,1,:)))']*BETA_contextsgbow;
    contextsgbow_pred2 = [1 squeeze(zscore(lo_context_svd(i,2,:)))' squeeze(zscore(lo_sg_svd(i,2,:)))' squeeze(zscore(lo_bow_svd(i,2,:)))']*BETA_contextsgbow;
    contextordersgbow_pred1 = [1 squeeze(zscore(lo_context_svd(i,1,:)))' squeeze(zscore(lo_order_svd(i,1,:)))' squeeze(zscore(lo_sg_svd(i,1,:)))' squeeze(zscore(lo_bow_svd(i,1,:)))']*BETA_contextordersgbow;
    contextordersgbow_pred2 = [1 squeeze(zscore(lo_context_svd(i,2,:)))' squeeze(zscore(lo_order_svd(i,2,:)))' squeeze(zscore(lo_sg_svd(i,2,:)))' squeeze(zscore(lo_bow_svd(i,2,:)))']*BETA_contextordersgbow;
    
    
    %get the values from data

    actual_vector1 = squeeze(zscore(lo_response2_100(i,1,:)));
    actual_vector2 = squeeze(zscore(lo_response2_100(i,2,:)));
    ...
    cor_contextsgbow = dot(contextsgbow_pred1, actual_vector1)/(norm(contextsgbow_pred1)*norm(actual_vector1)) + dot(contextsgbow_pred2, actual_vector2)/(norm(contextsgbow_pred2)*norm(actual_vector2));
    inc_contextsgbow = dot(contextsgbow_pred1, actual_vector2)/(norm(contextsgbow_pred1)*norm(actual_vector2)) + dot(contextsgbow_pred2, actual_vector1)/(norm(contextsgbow_pred2)*norm(actual_vector1));
    if(cor_contextsgbow > inc_contextsgbow)
        correct.('contextsgbow') = correct.('contextsgbow') + 1;
    end
    
    cor_contextordersgbow = dot(contextordersgbow_pred1, actual_vector1)/(norm(contextordersgbow_pred1)*norm(actual_vector1)) + dot(contextordersgbow_pred2, actual_vector2)/(norm(contextordersgbow_pred2)*norm(actual_vector2));
    inc_contextordersgbow = dot(contextordersgbow_pred1, actual_vector2)/(norm(contextordersgbow_pred1)*norm(actual_vector2)) + dot(contextordersgbow_pred2, actual_vector1)/(norm(contextordersgbow_pred2)*norm(actual_vector1));
    if(cor_contextordersgbow > inc_contextordersgbow)
        correct.('contextordersgbow') = correct.('contextordersgbow') + 1;
    end
end

save_path = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/results_svd_sv2_100.mat');
