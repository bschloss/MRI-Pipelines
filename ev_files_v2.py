...
def main():
    ...
    for d in edf_dirs:
        ...
        if ('021/EDF/Run4/' not in d or '2021/EDF/Run4/' in d) and '2005/EDF/Run2' not in d and '2006/EDF/Run5' not in d and '2014/EDF/Run5' not in d:
            ...
            evs = {}
            for key in ['Content_Word','Target_Word','Content_or_Target_Word', 'No_Interest']:
                evs[key] = ''
            ...
            while(edfws.cell(row=edf_row,column=1).value!=None):
                ...
                if not(sent_id==str(0) or sent_id=='.'):
                    ...
                    if not(fix_rel_onset>sent_dur):
                        ...
                        if area_id != str(0) and area_id != '.' and area_id != None and not (sent_id == 't.04.18' and area_id == str(14)):
                            ...
                            if regression == 1 or int(text_data.cell(row=textdata_row,column=8).value) == int(1):
                                val = "{0:.2f}".format(float(1)) + '\n'
                                evs['No_Interest'] += '\t'.join(["{0:.2f}".format(true_onset),"{0:.2f}".format(fix_duration),val])
                                evs['No_Interest_with_Words'] += '\t'.join([w,"{0:.2f}".format(true_onset),"{0:.2f}".format(fix_duration),val])
                            else:
                                if int(text_data.cell(row=textdata_row,column=9).value) == int(1):
                                    ...
                                    evs['Target_Word_with_Words'] += '\t'.join([w,"{0:.2f}".format(true_onset),"{0:.2f}".format(fix_duration),val])
                                    ...
                                    for key in evs.keys():
                                        if key not in ['Content_Word','Target_Word','Content_or_Target_Word','No_Interest'] and 'with_Words' not in key and 'Target' in key:
                                ...
                                if int(text_data.cell(row=textdata_row,column=5).value) == int(1):
                                    ...
                                    evs['Content_Word_with_Words'] += '\t'.join([w,"{0:.2f}".format(true_onset),"{0:.2f}".format(fix_duration),val])
                                    evs['Content_or_Target_Word'] += '\t'.join(["{0:.2f}".format(true_onset),"{0:.2f}".format(fix_duration),val])
                                    evs['Content_or_Target_Word_with_Words'] += '\t'.join([w,l,"{0:.2f}".format(true_onset),"{0:.2f}".format(fix_duration),val])
                                    
                                    for key in evs.keys():
                                        if key not in ['Content_Word','Target_Word','Content_or_Target_Word','No_Interest'] and 'with_Words' not in key and 'Content' in key:
                                            val = "{0:.2f}".format(float(text_data.cell(row=textdata_row,column=evs[key]['column']).value)) + '\n'
                                            evs[key]['text'] += '\t'.join(["{0:.2f}".format(true_onset),"{0:.2f}".format(fix_duration),val])
                edf_row += 1
            
            for key in evs.keys():
                if key in ['Content_Word','Target_Word','Content_or_Target_Word','No_Interest'] or 'with_Words' in key:                
            ...
            for suffix in ['Target','Content','Content_or_Target']:
                ...
                for i in range(len(evs['_'.join(['AoA',suffix])]['text'])):
                    zero_vals = 0
                    for key in evs.keys():
                        if key not in ['Content_Word', 'Target_Word',
                                       'Content_or_Target_Word',
                                       'No_Interest'] and 'with_Words' not in key and 'Dummy' not in key and 'Imageability' not in key:
                            if suffix == 'Content_or_Target' and 'Content_or_Target' in key:
                                line = evs['_'.join(['AoA',suffix])]['text'][i].split('\t')
                ...
                for line in remove_lines:
                    ...
                    val = "{0:.2f}".format(float(1)) + '\n'
                    evs['_'.join(['Dummy',suffix])] += evs['_'.join([suffix,'Word'])][line] + '\n'
                    evs['_'.join(['Dummy',suffix,'with_Words'])] += evs['_'.join([suffix,'Word','with_Words'])][line] + '\n'
                    #print suffix,evs['_'.join([suffix,'Word','with_Words'])][line],evs['_'.join([suffix,'Word','with_Words'])][line][-1]
                    for key in evs.keys():
                        if key not in ['Content_Word', 'Target_Word',
                                       'Content_or_Target_Word',
                                       'No_Interest'] and 'with_Words' not in key and 'Dummy' not in key:
                            if suffix == 'Content_or_Target' and 'Content_or_Target' in key:
                ...
                evs['_'.join(['Dummy',suffix])] = evs['_'.join(['Dummy',suffix])].rstrip('\n').split('\n')
                evs['_'.join(['Dummy',suffix])].reverse()
                evs['_'.join(['Dummy',suffix])] = '\n'.join(evs['_'.join(['Dummy',suffix])])
                evs['_'.join(['Dummy',suffix,'with_Words'])] = evs['_'.join(['Dummy',suffix,'with_Words'])].rstrip('\n').split('\n')
                evs['_'.join(['Dummy',suffix,'with_Words'])].reverse()
                evs['_'.join(['Dummy',suffix,'with_Words'])] = '\n'.join(evs['_'.join(['Dummy',suffix,'with_Words'])])

            for suffix in ['Target','Content','Content_or_Target']:
                i = 0
                while (i < len(evs['_'.join([suffix,'Word','with_Words'])])-1):
                    w1 = evs['_'.join([suffix,'Word','with_Words'])][i].split('\t')[0]
                    w2 = evs['_'.join([suffix,'Word','with_Words'])][i+1].split('\t')[0]
                    if w1==w2:
                        for key in evs.keys():
                            if len(key) > len(suffix):
                                if key[-1*len(suffix):] == suffix:
                                    if not (suffix == 'Target' and key[-1*len('Content_or_Target'):] == 'Content_or_Target'):
                                        if key not in ['Content_Word', 'Target_Word',
                                                       'Content_or_Target_Word',
                                                       'No_Interest'] and 'with_Words' not in key and 'Dummy' not in key and 'Imageability' not in key:
                                            #print key
                                            onset = evs[key]['text'][i].split('\t')[0]
                                            duration = float(evs[key]['text'][i+1].split('\t')[0])
                                            duration += float(evs[key]['text'][i+1].split('\t')[1])
                                            duration += -1*float(onset)
                                            duration = "{0:.2f}".format(duration)  
                                            pm = evs[key]['text'][i].split('\t')[2]
                                            evs[key]['text'][i] = '\t'.join([onset,duration,pm])
                                            del evs[key]['text'][i+1]
                        key = suffix + '_' + 'Word'
                        onset = evs[key][i].split('\t')[0]
                        duration = float(evs[key][i+1].split('\t')[0])
                        duration += float(evs[key][i+1].split('\t')[1])
                        duration += -1*float(onset)
                        duration = "{0:.2f}".format(duration) 
                        pm = "{0:.2f}".format(float(1))
                        evs[key][i] = '\t'.join([onset,duration,pm])
                        del evs[key][i+1]
                        key += '_with_Words'
                        l1 = evs[key][i].split('\t')[1]
                        evs[key][i] = '\t'.join([w1,l1,onset,duration,pm])
                        del evs[key][i+1]
            ...
            for key in evs.keys():
                if 'Dummy' in key:
                    open(''.join([EV_DIR,'Run',str(runnum),'/',key,'.run00',str(runnum),'.txt']),'w').write(evs[key])
                elif key in ['Content_Word','Target_Word','Content_or_Target_Word','No_Interest'] or 'with_Words' in key:
                    if(l2t(evs[key])!='\n' and l2t(evs[key])!=''):
