...
pardirs = [datadir + d + '/fMRI_Analyses/Full_Linear_ID/' for d in pardirs]
parcopes = []
parvarcopes = []
pardof = []
...
datasink.inputs.base_directory = datadir + '/Group_Analyses/Full_Linear_ID_MvB/'
...
analysis.connect(data,'con',flameo,'t_con_file')
analysis.connect(data,'design',flameo,'design_file')
analysis.connect(data,'grp',flameo,'cov_split_file')
...
fwe = MapNode(interface=Function(input_names=['zstat','mask'],
                                    ...
                                    name='FWE',
                                    iterfield=['zstat'])
analysis.connect(flameo,('zstats',unnest_list),fwe,'zstat')
...
fdr = MapNode(interface=Function(input_names=['zstat','mask'],
                                   ...
                                   name='FDR',
                                   iterfield=['zstat'])
analysis.connect(flameo,('zstats',unnest_list),fdr,'zstat')
...
analysis.connect(flameo,('zstats',unnest_list),smooth_est,'zstat_file')
...
analysis.connect(flameo,('zstats',unnest_list),cluster,'in_file')
...
cluster_no_pthresh = MapNode(interface=fsl.model.Cluster(minclustersize=True,
                                        ...
                                        iterfield = ['in_file'],
                                        iterables = ('threshold',[2.33,3.08,3.27,3.62]))
analysis.connect(flameo,('zstats',unnest_list),cluster_no_pthresh,'in_file')
