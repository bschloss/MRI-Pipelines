...
pardirs = [datadir + d + '/fMRI_Analyses/Low_Level_Linear/' for d in pardirs]
parcopes = []
parvarcopes = []
pardof = []
...
data = Node(ID(fields=['parvarcopes','parcopes','pardof','mask','struct','design_mono_gt_bi','design','grp','con'],mandatory_inputs=False),name='Data')
...
data.inputs.design = '/gpfs/group/pul8/default/read/mono_vs_bi.mat'
data.inputs.grp = '/gpfs/group/pul8/default/read/mono_vs_bi.grp'
data.inputs.con = '/gpfs/group/pul8/default/read/mono_vs_bi.con'
data.struct = [struct]

#Use the DataSink function to store all outputs
datasink = Node(nio.DataSink(), name= 'Output')
datasink.inputs.base_directory = datadir + '/Group_Analyses/Low_Level_Linear_MvB/'
...
analysis.connect(data,'con',parametric_flameo,'t_con_file')
analysis.connect(data,'design',parametric_flameo,'design_file')
analysis.connect(data,'grp',parametric_flameo,'cov_split_file')
...
parametric_fwe = MapNode(interface=Function(input_names=['zstat','mask'],
                                    ...
                                    name='Parametric_FWE',
                                    iterfield=['zstat'])
analysis.connect(parametric_flameo,('zstats',unnest_list),parametric_fwe,'zstat')
...
parametric_fdr = MapNode(interface=Function(input_names=['zstat','mask'],
                                   ...
                                   name='Parametric_FDR',
                                   iterfield=['zstat'])
analysis.connect(parametric_flameo,('zstats',unnest_list),parametric_fdr,'zstat')
...
analysis.connect(parametric_flameo,('zstats',unnest_list),parametric_smooth_est,'zstat_file')
...
analysis.connect(parametric_flameo,('zstats',unnest_list),parametric_cluster,'in_file')
...
parametric_cluster_no_pthresh = MapNode(interface=fsl.model.Cluster(minclustersize=True,
                                        ...
                                        iterfield = ['in_file'],
                                        iterables = ('threshold',[2.33,3.08,3.27,3.62]))
analysis.connect(parametric_flameo,('zstats',unnest_list),parametric_cluster_no_pthresh,'in_file')
