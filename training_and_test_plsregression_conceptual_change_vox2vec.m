...
context_designs = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/design_context.mat');
load(context_designs);
order_designs = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/design_order.mat');
load(order_designs);
contextorder_designs = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/design_contextorder.mat');
load(contextorder_designs);
sg_designs = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/design_sg.mat');
load(sg_designs);
bow_designs = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/design_bow.mat');
load(bow_designs);
sgbow_designs = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/design_sgbow.mat');
load(sgbow_designs);
responses = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/response.mat');
load(responses);
context_lo = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/lo_context.mat');
load(context_lo);
order_lo = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/lo_order.mat');
load(order_lo);
contextorder_lo = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/lo_contextorder.mat');
load(contextorder_lo);
sg_lo = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/lo_sg_svd(.mat');
load(sg_lo);
bow_lo = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/lo_bow.mat');
load(bow_lo);
sgbow_lo = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/lo_sg_svd(bow.mat');
load(sgbow_lo);
response_lo = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/lo_response.mat');
load(response_lo);

N = size(response, 1); 
ncomp = min(30,size(design_context,2)-2);
correct = struct();
correct.('context') = 0;
correct.('order') = 0;
...
for i = 1:N
    ...
    context_pred1 = [1 squeeze(zscore(lo_response(i,1,:)))']*BETA_context;
    context_pred2 = [1 squeeze(zscore(lo_response(i,2,:)))']*BETA_context;
    order_pred1 = [1 squeeze(zscore(lo_response(i,1,:)))']*BETA_order;
    order_pred2 = [1 squeeze(zscore(lo_response(i,2,:)))']*BETA_order;
    contextorder_pred1 = [1 squeeze(zscore(lo_response(i,1,:)))']*BETA_contextorder;
    contextorder_pred2 = [1 squeeze(zscore(lo_response(i,2,:)))']*BETA_contextorder;
    sg_pred1 = [1 squeeze(zscore(lo_response(i,1,:)))']*BETA_sg;
    sg_pred2 = [1 squeeze(zscore(lo_response(i,2,:)))']*BETA_sg;
    bow_pred1 = [1 squeeze(zscore(lo_response(i,1,:)))']*BETA_bow;
    bow_pred2 = [1 squeeze(zscore(lo_response(i,2,:)))']*BETA_bow;
    sgbow_pred1 = [1 squeeze(zscore(lo_response(i,1,:)))']*BETA_sgbow;
    sgbow_pred2 = [1 squeeze(zscore(lo_response(i,2,:)))']*BETA_sgbow;
    contextsgbow_pred1 = [1 squeeze(zscore(lo_response(i,1,:)))']*BETA_contextsgbow;
    contextsgbow_pred2 = [1 squeeze(zscore(lo_response(i,2,:)))']*BETA_contextsgbow;
    contextordersgbow_pred1 = [1 squeeze(zscore(lo_response(i,1,:)))']*BETA_contextordersgbow;
    contextordersgbow_pred2 = [1 squeeze(zscore(lo_response(i,2,:)))']*BETA_contextordersgbow;
    ...
    actual_sgbow_vector1 = squeeze(zscore(lo_sg_svd(bow(i,1,:)));
    actual_sgbow_vector2 = squeeze(zscore(lo_sg_svd(bow(i,2,:)));
...
save_path = strcat('/gpfs/group/pul8/default/read/',par,'/fMRI_Analyses/Conceptual_Change/results_vox2vec.mat');
