...
pardirs = [datadir + d + '/fMRI_Analyses/' for d in pardirs]
parcopes = []
parvarcopes = []
pardof = []
for i in range(38):
...
data = Node(ID(fields=['parvarcopes','parcopes','pardof','mask'],mandatory_inputs=False),name='Data')
...
datasink.inputs.base_directory = '/'.join(datadir.split('/')[:-2]) + '/Group_Analyses/'
...
analysis.connect(parametric_fdr,'fwe_corrected',datasink,'Parametric_FDR')
